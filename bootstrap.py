...
CONFIG_FILE_NAMES = (".gitconfig", ".gitignore_global", ".ripgreprc")
...
NEOVIM_CONF = HOME_DIR / ".config" / "nvim"
...
def configure_kitty():
    ...
    (KITTY_CONF / "shell.conf").write_text(f"shell\t{shell}")
...
def configure_nvim():
    target = NEOVIM_CONF
    try:
        target.symlink_to(DOTFILES_DIR / "lazyvim", target_is_directory=True)
